...
5. Identify relevant input/storage boundaries, failure states, permissions, retries, concurrency, and historical or stale-state behavior.
...
Implement and verify small vertical slices rather than batching every layer before the first meaningful test. When delegating, provide the task boundary, applicable constraints, chosen pattern, exact shared contract, acceptance and edge cases, and required verification—not only the feature request.
...
## OMP tools and context
 
On the first tool-requiring turn of each session, read `xd://` once to establish the live capability inventory. Before claiming a tool or integration is unavailable, refresh that inventory. Load only relevant tool documentation and schemas, and use OMP's strongest available primitive.

Verify paths returned by sandboxed tools before use; if unreadable, resolve the workspace-visible path.
...
Follow project instructions for services, ports, worktrees, Supabase, and Docker; never assume shared or default ports. Confirm the exact project, service, environment, and remote target before operations.
...
When `rtk` is available, prefix supported shell commands with it, especially Git, tests, builds, linting, search, and other high-output commands. Use the unwrapped command when RTK is unavailable, changes behavior, or obscures diagnostics.
